/** @file bms_config.h @brief 电芯串数、容量、化学体系和保护阈值集中配置。 @details 电压单位 V、电流单位 A、温度单位 °C。 @par 使用方法 在工程配置阶段选择 INIT_* 宏。 */
...
/***************************** 电池保护相关参数 ***********************************/
// 三元锂电?Ternary lithium battery)默认参数
...
// 钛酸锂电?Lithium titanate battery)默认参数
...
#define INIT_OV_PROTECT			TLB_OV_PROTECT			// 单体过压保护电压(V)，注意BQ769X0的OV配置范围。
#define INIT_OV_RELIEVE			TLB_OV_RELIEVE			// 单体过压恢复电压(V)
#define INIT_UV_PROTECT			TLB_UV_PROTECT			// 单体欠压保护电压(V)，注意BQ769X0的UV配置范围。
...
#define INIT_OTC_PROTECT	70		// 充电过温保护(°C)，OTC:Over Temperature Charge
#define INIT_OTC_RELIEVE	60		// 充电过温解除温度(°C)

#define INIT_OTD_PROTECT	70		// 放电过温保护(°C)，OTD:Over Temperature Discharge
#define INIT_OTD_RELIEVE	60		// 放电过温解除温度(°C)

#define INIT_LTC_PROTECT	-20		// 充电低温保护(°C)，LTC:Low Temperature Charge
#define INIT_LTC_RELIEVE	-10		// 充电低温解除温度(°C)

#define INIT_LTD_PROTECT	-20		// 放电低温保护(°C)，LTD:Low Temperature Discharge
#define INIT_LTD_RELIEVE	-10		// 放电低温解除温度(°C)
...
#define SOC_STOP_CHG_VALUE		1		// 停止充电的SOC阈值。
#define SOC_START_CHG_VALUE		0.99	// 启动充电的SOC阈值。
#define SOC_STOP_DSG_VALUE		0		// 停止放电的SOC阈值。
#define SOC_START_DSG_VALUE		0.01	// 启动放电的SOC阈值。
